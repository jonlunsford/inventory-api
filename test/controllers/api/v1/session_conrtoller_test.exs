defmodule Inventory.Api.V1.SessionControllerTest do
  ...
  test "POST /api/v1/token", %{conn: conn} do
    conn = post conn, api_v1_login_path(conn, :create), %{grant_type: "password", email: "test@example.com", password: "password"}
    assert json_response(conn, 200)["access_token"]
  end

  test "POST /api/v1/token with invalid email", %{conn: conn} do
    conn = post conn, api_v1_login_path(conn, :create), %{grant_type: "password", email: "test@example.com", password: "pass"}
